...
function attackCloseFromSpawnEnemies(creep: Creep, spawn: StructureSpawn): boolean {
    ...
    if (target) {
        if (getRange(spawn, target) <= 20) {
            const {executed} = action<CreepAttackResult>(() => creep.attack(target), { [ERR_NOT_IN_RANGE]: () => {action(() => creep.moveTo(target))} });
            return executed;
        } 
    }
    return false;
...
function destroyBlockingWalls(creep: Creep, spawn: StructureSpawn): boolean {
    ...
    if (walls.length > 0) {
        ...
        const {executed} = action<CreepAttackResult>(() => creep.attack(targetWall), { [ERR_NOT_IN_RANGE]: () => {action(() => creep.moveTo(targetWall))} });
        return executed;
    }
    return false;
}